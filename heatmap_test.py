...
def meth_browser(
    ...
    )  # return window = soms nodig wanneer window uit methfreqtable wordt gehaald voor annotatie
...
def parse_nanopolish(files, table, names, window, groups, outtable):
    ...
    """
    if table:
        files, names = parse_overviewtable_nanopolishfiles(table)
...
def parse_methfrequencytable(table, names, window, groups, gff, outtable):
    ...
    if value2 > value1:
        methfreqtable = table.sort_values("position")
    else:
        methfreqtable = table
    ...
    if (
        ...
        if gff:
            ...
            if "chrom" in methfreqtable.columns:
                ...
                logging.error(
                    "Error when extracting window out of methfreqtable positions for annotation."
                )
                sys.stderr.write(
                    "\n\nError when extracting window out of methfreqtable positions for annotation.\n"
                )
                sys.stderr.write(
                    "Is chromsome present in the position information of the input table?"
                )
