...
df["end"] = df["end"].astype(int)  # modkit files are zero based, so to make it 1-based i take the end position
...
while current_chunk_start < max_chunk_end:
    ...
    chunk_df = df[(df["end"] >= current_chunk_start) & (df["end"] < current_chunk_end)]
    if chunk_df.empty:
