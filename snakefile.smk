...
sample_info_baminput = pd.read_table("/home/ecoopman/heatmap/baminput/bamfiles/bamfiles.txt").set_index("SAMPLE", drop=False)
...
rule heatmap:
    input:
        ...
        bam = expand(os.path.join("/home/ecoopman/heatmap/baminput/bamfiles/{id}.bam"), id=sample_info_baminput),
    ...
    shell:
         "python heatmap.py --files {input.bam} --window chr17:44345246-44353106 --fasta {input.fasta} --outfig {output.outfig} --outtable {output.outtable} 2> {log}"
